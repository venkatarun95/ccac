''' A version of BBR that computes rate over a full RTT '''
...
class BBRVariables:
    ...
    def __init__(self, c: ModelConfig, s: MySolver):
        ...
        self.r_max_f = [[s.Real(f"bbr_max_rate_est_{n},{t}")
                         for t in range(c.T)] for n in range(c.N)]
        # A(t) at the last RTT switch (notice, time is continuous)
        self.last_A_f = [[s.Real(f"bbr_last_arrival_{n},{t}")
                          for t in range(c.T)]
                         for n in range(c.N)]
        # S(t-R) at the last switch
        self.last_S_f = [[s.Real(f"bbr_last_service_{n},{t}")
                          for t in range(c.T)]
                         for n in range(c.N)]
...
def cca_bbr_rate_est(b: BBRVariables, c: ModelConfig,
                     s: MySolver, v: Variables):
...
def cca_bbr_max_rate(b: BBRVariables, c: ModelConfig,
                     s: MySolver, v: Variables):
    for n in range(c.N):
...
class TestBBR(unittest.TestCase):
    @staticmethod
    def util_net_without_cca(
            c: ModelConfig
    ) -> Tuple[BBRVariables, MySolver, Variables]:
    ...
    def test_bbr_rate_est(self):
        ...
        for t in range(c.T):
            print(t, m[f"bbr_state_0,{t}"], m[f"bbr_last_arrival_0,{t}"],
                  m[f"bbr_rate_est_0,{t}"], m[f"bbr_last_service_0,{t}"])
        plot_model(m, c)
